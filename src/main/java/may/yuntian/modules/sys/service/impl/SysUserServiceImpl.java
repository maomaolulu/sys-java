...
//			user.setPassword(newCommission Sha256Hash(user.getPassword(), user.getSalt()).toHex());
		}
...
//		checkRole(user);
	...
	@Override
	public boolean updateLoginPassword(String email, String password, String newPassword,Long userId) {
        SysUserTokenService sysUserTokenService = SpringContextUtils.getBean("sysUserTokenService",SysUserTokenService.class);
        sysUserTokenService.deleteToken(userId);
		SysUserEntity userEntity = new SysUserEntity();
		userEntity.setPassword(newPassword);
