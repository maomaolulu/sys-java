...
import may.yuntian.modules.sys_v2.exception.ServiceException;
import may.yuntian.modules.sys_v2.mapper.SysRoleDeptMapper;
 ...
 */
@Service
public class SysRoleV2ServiceImpl implements SysRoleV2Service {
    ...
    public SysRoleV2ServiceImpl(SysRoleMapper roleMapper, SysRoleMenuMapper roleMenuMapper, SysRoleDeptMapper roleDeptMapper, SysUserRoleMapper userRoleMapper) {
...
//        }
        for (SysRole perm : perms) {
            if (StringUtils.isNotNull(perm)) {
                ...
                if (StringUtils.isNotEmpty(roleKey)) {
                    permsSet.addAll(Arrays.asList(roleKey.trim().split(",")));
                }
     ...
     * 新增保存角色信息
     *
     * @param roleVo 角色信息数据体
     * @return 结果
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int insertRole(SysRoleVo roleVo) {
     ...
     */
    @Override
    public boolean checkUserRole(Long userId, String roleKey, String system) {
        return userRoleMapper.checkUserRole(userId, roleKey, system);
    }
