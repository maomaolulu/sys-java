...
//        }
     ...
     *
     * @param file 文件
     * @param type 存储路径类型  检评 zj_app 评价 pj_app
     * @return
     */
    public static String uploadStatic(MultipartFile file,String type) {
//        String bucketName = prop.getBucketName();
//        System.out.println("bucketName = " + bucketName);
        String staticBucketName = "static";
        String originalFilename = file.getOriginalFilename();
        if (StringUtils.isBlank(originalFilename)) {
            throw new RuntimeException();
        }
        ...
        try {
            InputStream in = file.getInputStream();
            PutObjectArgs objectArgs = PutObjectArgs.builder().bucket(staticBucketName).object(objectName)
                    .stream(in, file.getSize(), -1).contentType(file.getContentType()).build();
            boolean exist = bucketExists(staticBucketName);
            if (!exist) {
                makeBucket(staticBucketName);
            }
            //文件名称相同会覆盖
            minioClient.putObject(objectArgs);
            in.close();
        ...
        System.out.println("bucketName = " + staticBucketName + "/" + objectName);
        return staticBucketName + "/" + objectName;
    }
     ...
     * @return
     */
    public static String upload2(MultipartFile file,String type,String fileName) {
//        String bucketName = prop.getBucketName();
        System.out.println("bucketName = " + bucketName);
        String originalFilename = file.getOriginalFilename();
        if (StringUtils.isBlank(originalFilename)) {
...
//        String bucketName = prop.getBucketName();
     ...
     * @return Boolean
     */
    public static Integer download(String fileName, HttpServletResponse res) {
//        String bucketName = prop.getBucketName();
     ...
     * @return Boolean
     */
    public static Integer download2(String fileName, HttpServletResponse res,String name) {
//        String bucketName = prop.getBucketName();
        System.out.println("fileName = " + fileName);
        ...
        try (InputStream response = minioClient.getObject(objectArgs)) {
            ...
            try (FastByteArrayOutputStream os = new FastByteArrayOutputStream()) {
                ...
                // 设置强制下载不打开
                // res.setContentType("application/force-download");
                res.addHeader("Content-Disposition", "attachment;filename=" .concat(String.valueOf(URLEncoder.encode(name, "UTF-8"))));
                try (ServletOutputStream stream = res.getOutputStream()) {
                    stream.write(bytes);
                    stream.flush();
                }
