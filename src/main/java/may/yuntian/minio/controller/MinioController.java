...
public class MinioController {
    ...
    @ApiOperation(value = "静态文件上传返回url")
    @PostMapping("/uploadStatic")
    public Result uploadStatic(@RequestParam("file") MultipartFile file,@RequestParam("type") String type) {
        String objectName = MinioUtil.uploadStatic(file,type);
        if (null != objectName) {
            alRedisUntil.hset("anlian-java",objectName, DateUtils.getDate());
            return Result.ok().put("path", objectName);
        }
