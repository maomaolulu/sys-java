...
public interface PerformanceAllocationMapper extends BaseMapper<PerformanceAllocationEntity> {
    ...
    @Select("SELECT pa.id as id,p.id as projectId,p.charge as charge,p.total_money as totalMoney,p.company as company,p.identifier as identifier,p.netvalue as netvalue,p.included_output as includedOutput,pa.types as types,pa.performance_money as performanceMoney, " +
    ...
    PerformanceAllocationNewVO getMapByQurryWrapper(@Param(Constants.WRAPPER) Wrapper<PerformanceAllocationEntity> userWrapper);
