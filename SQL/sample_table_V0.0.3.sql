...
-- t_air_fixed	空气中有害物质定点采样记录表  包含个体和定点
DROP TABLE IF EXISTS t_air_fixed;

-- t_air_fixed	空气中粉尘分散度定点采样记录表  暂时废弃
...
`id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '自增主键ID',
 ...
 `calib_model_id` VARCHAR(200) COMMENT '声校准器型号/编号',
  `instrument_code` VARCHAR(100) COMMENT '仪器编号',
...
`id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '自增主键ID',
 ...
 `profession` VARCHAR(100) COMMENT '工种',
 `place` VARCHAR(200) COMMENT '采样地点',
  `instrument_code` VARCHAR(100) COMMENT '采样仪器编号',
   `equip` VARCHAR(100) COMMENT '生产设备型号和参数', 
 `touch_time` FLOAT COMMENT '接触时间(h/d)',
 `begin_time` VARCHAR(50) COMMENT '采样开始时间',
 `end_time` VARCHAR(50) COMMENT '采样结束时间',
 `test_time` INT COMMENT '测定时间',
...
`id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '自增主键ID',
 ...
 `place` VARCHAR(200) COMMENT '采样地点',
  `instrument_code` VARCHAR(100) COMMENT '采样仪器编号',
  `equip` VARCHAR(100) COMMENT '设备名称和频率范围',  
 `touch_time` FLOAT COMMENT '接触时间(h/d)',
 `begin_time` VARCHAR(50) COMMENT '采样开始时间',
 `end_time` VARCHAR(50) COMMENT '采样结束时间',
 `test_time` INT COMMENT '测定时间',
...
`id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '自增主键ID',
 ...
 `place` VARCHAR(200) COMMENT '采样地点',
  `instrument_code` VARCHAR(100) COMMENT '仪器编号',
  `equip` VARCHAR(100) COMMENT '设备（生产设备）',  
 `touch_time` FLOAT COMMENT '接触时间(h/d)',
 `begin_time` VARCHAR(50) COMMENT '采样开始时间',
 `end_time` VARCHAR(50) COMMENT '采样结束时间',
 `test_time` INT COMMENT '测定时间',
...
`id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '自增主键ID',
 ...
 `place` VARCHAR(200) COMMENT '采样地点',
 `people` VARCHAR(50) COMMENT '佩戴人姓名',
 `type` INT DEFAULT 0 COMMENT '采样方式（0无，1定点，2个体）',
 `substance` VARCHAR(200) COMMENT '检测物质',
 -- `substance_id` BIGINT NOT NULL COMMENT '检测物质ID',
 `ismerge` BOOLEAN DEFAULT 0 NULL COMMENT '是否需要合并的数据(0:单一物质,1多物质界面需要合并)',
 `sample_code` VARCHAR(50) COMMENT '样品编号',
 `instrument_code` VARCHAR(50) COMMENT '仪器编号',
 `protective_measures` VARCHAR(500) COMMENT '生产情况、个人防护用品使用情况',
 `before_flow` DECIMAL(10,2) COMMENT '采样前流量(L/min)',
 `after_flow` DECIMAL(10,2) COMMENT '采样后流量(L/min)',
 `begin_time` VARCHAR(50) COMMENT '采样开始时间',
 `end_time` VARCHAR(50) COMMENT '采样结束时间',
 `test_time` INT COMMENT '测定时间',
